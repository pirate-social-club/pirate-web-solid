...
function SidebarFooter(props: { readonly signedIn: () => boolean; readonly onSignIn: () => void }) {
  return <div class="flex flex-col gap-3">
    ...
    <Show when={props.signedIn()} fallback={<button type="button" onClick={props.onSignIn} onFocus={prepareSignIn} onPointerDown={prepareSignIn} onPointerEnter={prepareSignIn} class="block w-full rounded-lg bg-white px-3 py-2 text-center text-sm font-semibold text-black hover:bg-white/90">Sign in</button>}>
      ...
      <SidebarContent class="min-h-screen bg-background pb-20 md:pb-0">
        <div class="md:hidden">
          <AppHeader
            ...
            mobileTrailingContent={signedIn() ? undefined : <button type="button" onClick={openAuth} onFocus={prepareSignIn} onPointerDown={prepareSignIn} onPointerEnter={prepareSignIn} class="px-2 text-sm font-semibold text-white">Sign in</button>}
            onHomeClick={goHome}
