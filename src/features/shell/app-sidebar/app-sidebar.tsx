...
export function AppSidebar(props: AppSidebarProps) {
  ...
  return <aside aria-label={props.brandLabel ?? "Pirate navigation"} class={cn("flex min-h-screen w-[15.5rem] shrink-0 flex-col border-e border-sidebar-border bg-sidebar p-4 text-sidebar-foreground", props.appearance === "media" && "bg-black text-white", props.collapsed && "w-20 px-2", props.class)}>
    <button aria-label={props.homeAriaLabel ?? "Go to home"} class="mb-5 flex items-center gap-3 rounded-lg px-2 py-2 text-start focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={props.onHomeClick} type="button"><span aria-hidden="true" class="grid size-9 shrink-0 place-items-center rounded-full border border-white/25 bg-white/10 font-display text-sm font-semibold">P</span><Show when={!props.collapsed}><Type as="span" variant="h4" class="font-display tracking-wide">{props.brandLabel ?? "PIRATE"}</Type></Show></button>
