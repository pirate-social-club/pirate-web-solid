...
function PickerStory(props: Partial<PersonaSwitcherSheetProps>) {
  ...
  return <><Button onClick={() => setOpen(true)}>Switch profile</Button><PersonaSwitcherSheet
    ...
    onSelect={id => { setSelected(id); setOpen(false); }} onSettings={() => setOpen(false)} onViewProfile={() => setOpen(false)} {...props}
  /></>;
}
...
export const Loading: Story = { render: () => <PickerStory personas={[]} loading onViewProfile={undefined} /> };
export const Unavailable: Story = { render: () => <PickerStory personas={[]} unavailable onRetry={() => {}} onViewProfile={undefined} /> };
export const Empty: Story = { render: () => <PickerStory personas={[]} onViewProfile={undefined} /> };
