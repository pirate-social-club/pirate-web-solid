...
const meta = {
  title: "App/Bookings/BookingSessionControls",
...
export const HostAfterSession: Story = { args: { state: "ready-to-settle", viewerRole: "host" }, render: () => <InteractiveSession state="ready-to-settle" viewerRole="host" />, play: async ({ canvasElement }) => { const canvas = within(canvasElement); await userEvent.click(canvas.getByRole("button", { name: "Finish" })); await expect(canvas.getByText("finish")).toBeInTheDocument(); await userEvent.click(canvas.getByRole("button", { name: "Report" })); await expect(canvas.getByText("report")).toBeInTheDocument(); } };
