...
const meta = { title: "App/Bookings/HostBookingPage", component: HostBookingPage, args: profile, parameters: { layout: "fullscreen", a11y: { test: "error" } } } satisfies Meta<typeof HostBookingPage>;
export default meta;
...
export const WithAvailability: Story = { render: () => frame(<InteractiveHostBooking />), play: async ({ canvasElement }) => { const canvas = within(canvasElement); await userEvent.click(canvas.getByRole("button", { name: "Book" })); await expect(canvas.getByText("book")).toBeInTheDocument(); } };
