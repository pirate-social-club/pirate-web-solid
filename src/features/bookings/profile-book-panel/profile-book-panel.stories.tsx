...
export const ModeTransition: Story = { args: { ...base, mode: "owner", configured: true, basePriceCents: 5000, slots, onEdit: () => {} }, render: () => <InteractiveModeTransition />, play: async ({ canvasElement }) => { const canvas = within(canvasElement); const panel = () => canvasElement.querySelector<HTMLElement>("[data-profile-book-panel]"); const initialPanel = panel(); if (!initialPanel) throw new Error("Profile booking panel did not render"); await expect(initialPanel).toHaveAttribute("data-profile-book-panel", "owner-configured"); await userEvent.click(canvas.getByRole("button", { name: "Toggle booking perspective" })); const viewerPanel = panel(); if (!viewerPanel) throw new Error("Profile booking panel disappeared"); await expect(viewerPanel).toHaveAttribute("data-profile-book-panel", "viewer"); await expect(within(viewerPanel).getByRole("button", { name: /11:00 AM.*\$50/ })).toBeInTheDocument(); } };
export const ViewerNoAvailability: Story = { args: { ...base, mode: "viewer", startingPriceCents: 5000, slots: [], onSelectSlot: () => {} } };
export const Mobile: Story = { args: { ...base, mode: "viewer", startingPriceCents: 5000, slots, onSelectSlot: () => {} }, globals: { viewport: { value: "mobile1", isRotated: false } } };
