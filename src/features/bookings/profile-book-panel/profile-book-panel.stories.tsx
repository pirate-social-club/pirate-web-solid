...
const meta = { title: "App/Bookings/ProfileBookPanel", component: ProfileBookPanel, parameters: { layout: "padded", a11y: { test: "error" } } } satisfies Meta<typeof ProfileBookPanel>;
export default meta;
...
export const OwnerConfigured: Story = { args: { ...base, mode: "owner", configured: true, basePriceCents: 5000, slots, onEdit: () => {} }, render: () => <InteractiveOwner />, play: async ({ canvasElement }) => { const canvas = within(canvasElement); await userEvent.click(canvas.getByRole("button", { name: "Edit" })); await expect(canvas.getByText("edit")).toBeInTheDocument(); } };
