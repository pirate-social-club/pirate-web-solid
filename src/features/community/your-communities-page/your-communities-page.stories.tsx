...
const meta = { title: "Compositions/Community/YourCommunitiesPage", component: YourCommunitiesPageView, args: { ...base, followingCommunities: communities, joinedCommunities: [communities[0]!], onCreateCommunity: () => undefined, onSelectCommunity: () => undefined }, parameters: { layout: "fullscreen" } } satisfies Meta<typeof YourCommunitiesPageView>;
export default meta;
type Story = StoryObj<typeof meta>;
export const Default: Story = { render: () => <StoryView {...base} followingCommunities={communities} joinedCommunities={[communities[0]!]} />, play: async ({ canvasElement }) => { const canvas = within(canvasElement); await userEvent.click(canvas.getByRole("button", { name: /Signal Room/ })); await expect(canvas.getByText(/selected Signal Room/)).toBeInTheDocument(); } };
