...
function resolveTotalBalanceLabel(props: WalletHubProps, readySections: WalletHubChainSection[]): string | null {
  if (props.totalBalanceUsd !== undefined) return props.totalBalanceUsd;
  if (props.assetsUnavailable || readySections.some(section => section.balancesUnavailable)) return null;
