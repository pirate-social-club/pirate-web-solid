...
export function winningViews(credits: readonly RewardCredit[]): readonly WinningView[] {
  return credits.flatMap((credit): WinningView[] => {
    ...
    const base = { creditId: credit.credit_id, amount, canSend: false };
    ...
    switch (claim.payout_status) {
      case "confirmed":
        return [{ ...base, status: "Sent to your wallet", detail: null, canClaim: false, canSend: canSendWinning(credit) }];
