...
async function standaloneMiddleware(request: Request, next: () => Promise<Response>) {
  ...
  const stagingPrivyRoute = pathname === "/staging/self-verify" || pathname === "/staging/funding-quote";
  const policy = verificationRoute
    ...
    : stagingPrivyRoute
      ? `default-src 'self'; script-src 'nonce-${nonce}' 'strict-dynamic'; img-src 'self' data:; frame-src https://auth.privy.io; connect-src 'self' https://auth.privy.io; worker-src 'self' blob:; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'`
