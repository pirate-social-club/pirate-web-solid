...
async function standaloneMiddleware(request: Request, next: () => Promise<Response>) {
  ...
  const policy = securityPolicy(new URL(request.url).pathname, nonce);
  headers.set("content-security-policy", policy);
  return new Response(response.body, { status: response.status, statusText: response.statusText, headers });
}

export function securityPolicy(pathname: string, nonce: string): string {
  ...
  return verificationRoute
    ...
    : veryRoute
      ...
      : signInRoute
        ...
        : `default-src 'self'; script-src 'nonce-${nonce}' 'strict-dynamic'; frame-src https://auth.privy.io; connect-src 'self' https://auth.privy.io ${MEDIA_UPLOAD_ORIGIN}; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'`;
}
