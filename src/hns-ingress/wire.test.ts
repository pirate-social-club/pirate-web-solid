...
describe("interactive HNS ingress wire", () => {
  it("pins the exact profile bytes and digest", async () => {
    expect(encoder.encode(HNS_COMMUNITY_APP_INTERACTIVE_PROFILE_V3)).toHaveLength(
      HNS_COMMUNITY_APP_INTERACTIVE_PROFILE_V3_BYTES,
    );
    expect(await sha256Hex(encoder.encode(HNS_COMMUNITY_APP_INTERACTIVE_PROFILE_V3))).toBe(
      HNS_COMMUNITY_APP_INTERACTIVE_PROFILE_V3_SHA256,
    );
  });
