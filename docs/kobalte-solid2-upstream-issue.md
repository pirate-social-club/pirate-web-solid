# Upstream issue brief: Kobalte default Button and Solid 2 hydration

Status: **prepared locally; not filed externally**

Proposed title: `Default Button loses hydration for reactive children under Solid 2 RC`
...
- Vite `8.x`
- Cloudflare Worker adapter with SSR, CSP nonce middleware, and hydration
...
not attach its handler or update its child after hydration. The failure is
reproducible in the app integration path under the real Worker and enforced
CSP; it was not caused by the app's ordinary attribute forwarding.
...
The app currently carries a Bun patch for the exact `@kobalte/core@2.0.0-alpha.0`
package. It excludes `children` and `as` from forwarded props, renders the
...
remove the local patch once a released fix is available.
