...
try {
  ...
  assert.equal(await panel.innerText(), recordsText);
  assert.equal(writes().length, 2);
  await page.reload();
  await page.getByRole("heading", { name: "Your records are ready to publish" }).waitFor();
  assert.equal(await publish.isDisabled(), true);
  failReads = true;
  await page.getByText("Could not update progress. Try again using the same button.").waitFor();
  assert.equal(await publish.isEnabled(), true);
  assert.deepEqual(await panel.boundingBox(), recordsBox);
  assert.equal(writes().length, 2);
  failReads = false;
  await publish.click();
  await page.waitForFunction(() => !document.body.textContent.includes("Could not update progress."));
  assert.equal(await publish.isDisabled(), true);
  ...
  console.log(JSON.stringify({ ok: true, preparationGets: true, oneAcknowledgement: true, reloadResumes: true, stablePendingAndFailureLayout: true, noCountdown: true }));
} finally {
  await browser.close();
